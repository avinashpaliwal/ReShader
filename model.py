...
class up(nn.Module):
    ...
    def __init__(self, inChannels, outChannels, vChannels=0):
        """
        Parameters
        ----------
            inChannels : int
        ...
        self.conv2 = nn.Conv2d(2 * outChannels+vChannels, outChannels, 3, stride=1, padding=1)
           
    def forward(self, x, skpCn, v=None):
        """
        Returns output tensor after passing input `x` to the neural network
        block.
        ...
        if v is None:
            ...
            n, _, h, w = x.shape
            v = v.expand(n, self.vChannels, h, w)
            x = F.leaky_relu(self.conv2(torch.cat((x, skpCn, v), 1)), negative_slope = 0.1)
        return x
...
class UNet(nn.Module):
    ...
    def __init__(self, inChannels, outChannels, vChannels=128):
        """
        Parameters
        ----------
            inChannels : int
...
class FCN(nn.Module):
    ...
    def forward(self, inp):
        ...
        return torch.cat((self.model(inp), inp), 1)