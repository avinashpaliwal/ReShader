...
def unproject_pts_pt(intrinsics, coords, depth, normalize_depth=False):
    if coords.shape[-1] == 2:
        coords = homogenize_pt(coords)
    intrinsics = intrinsics.squeeze()[:3, :3]
    coords = torch.inverse(intrinsics).mm(coords.T)
    if normalize_depth:
        coords = F.normalize(coords, dim=0, p=2.0)
        print("erer")
    coords = coords * depth.reshape(1, -1)
    return coords.T   # [n, 3]
